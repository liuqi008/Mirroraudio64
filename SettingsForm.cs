...
namespace MirrorAudio
{
    ...
    {
        // 手动输入队列参数控件
        NumericUpDown numMainMult = new NumericUpDown();
        NumericUpDown numMainFloor = new NumericUpDown();
        CheckBox chkMainReadFully = new CheckBox();

        NumericUpDown numAuxMult = new NumericUpDown();
        NumericUpDown numAuxFloor = new NumericUpDown();
        CheckBox chkAuxReadFully = new CheckBox();
        ...
        readonly ComboBox cmbResampAux  = new ComboBox();
        ...
        {
            _statusProvider = statusProvider ?? (new Func<StatusSnapshot>(() => new StatusSnapshot { Running = false }));
            ...
            var gIn = new GroupBox { Text = "输入（环回/录音）格式策略", Dock = DockStyle.Top, AutoSize = true, Padding = new Padding(10) };
            ...
            AddRow(tIn, "环回/录音请求策略", cmbInStrategy);
            ...
            // 手动输入队列参数（主）
            numMainMult.Minimum = 1; numMainMult.Maximum = 8; numMainMult.Value = 4; numMainMult.DecimalPlaces = 0;
            numMainFloor.Minimum = 1; numMainFloor.Maximum = 500; numMainFloor.Value = 80; numMainFloor.Increment = 1;
            chkMainReadFully.Text = "补齐填0（ReadFully）";

            // 读入当前设置
            numMainMult.Value = Clamp(cur.MainBufMult, 1, 8);
            numMainFloor.Value = Clamp(cur.MainBufFloorMs, 1, 500);
            chkMainReadFully.Checked = cur.MainReadFully;

            AddRow(tMain, "输入队列倍数", numMainMult);
            AddRow(tMain, "输入队列兜底 (ms)", numMainFloor);
            AddRow(tMain, "是否补齐填0", chkMainReadFully);
            ...
            // 手动输入队列参数（副）
            numAuxMult.Minimum = 1; numAuxMult.Maximum = 8; numAuxMult.Value = 4; numAuxMult.DecimalPlaces = 0;
            numAuxFloor.Minimum = 1; numAuxFloor.Maximum = 500; numAuxFloor.Value = 80; numAuxFloor.Increment = 1;
            chkAuxReadFully.Text = "补齐填0（ReadFully）";

            numAuxMult.Value = Clamp(cur.AuxBufMult, 1, 8);
            numAuxFloor.Value = Clamp(cur.AuxBufFloorMs, 1, 500);
            chkAuxReadFully.Checked = cur.AuxReadFully;

            AddRow(tAux, "输入队列倍数", numAuxMult);
            AddRow(tAux, "输入队列兜底 (ms)", numAuxFloor);
            AddRow(tAux, "是否补齐填0", chkAuxReadFully);
            ...
            {
                bool cust = (cmbInStrategy.SelectedIndex == (int)InputFormatStrategy.Custom);
                numInRate.Enabled = cust; numInBits.Enabled = cust;
            };
        ...
        void LoadConfig(AppSettings cur)
        {
            ...
            cmbResampMain.SelectedItem = (cur.MainResamplerQuality == 0 ? "60" : cur.MainResamplerQuality.ToString());
        ...
        void RenderStatus()
        {
            ...
            lblMainBuf.Text = (s.MainBufferRequestedMs > 0 ? (s.MainBufferRequestedMs + " ms → ") : "") +
                              (s.MainBufferMs > 0 ? (s.MainBufferMs + " ms") : "-") +
                              (s.MainAlignedMultiple > 0 ? (" (对齐≈" + s.MainAlignedMultiple.ToString("0.#") + "×)") : "");
            lblAuxBuf.Text  = (s.AuxBufferRequestedMs > 0 ? (s.AuxBufferRequestedMs + " ms → ") : "") +
                              (s.AuxBufferMs > 0 ? (s.AuxBufferMs + " ms") : "-") +
                              (s.AuxAlignedMultiple > 0 ? (" (对齐≈" + s.AuxAlignedMultiple.ToString("0.#") + "×)") : "");
        ...
        string BuildStatusText()
        {
            ...
            sb.AppendLine("主缓冲: " + (s.MainBufferRequestedMs > 0 ? (s.MainBufferRequestedMs + " ms → ") : "") + (s.MainBufferMs > 0 ? (s.MainBufferMs + " ms") : "-") + (s.MainAlignedMultiple > 0 ? (" (对齐≈" + s.MainAlignedMultiple.ToString("0.#") + "×)") : ""));
            ...
            sb.AppendLine("副缓冲: " + (s.AuxBufferRequestedMs > 0 ? (s.AuxBufferRequestedMs + " ms → ") : "") + (s.AuxBufferMs > 0 ? (s.AuxBufferMs + " ms") : "-") + (s.AuxAlignedMultiple > 0 ? (" (对齐≈" + s.AuxAlignedMultiple.ToString("0.#") + "×)") : ""));
